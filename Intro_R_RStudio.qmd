...
(The `<-` symbol consists of a less-than symbol `<` and a minus sign or dash, `-`, written next to each other. Together, they look like an arrow pointing towards the left. It is also possible to create this symbol by pressing Alt and - together. More on `<-` in @sec-assignment.)
