...
The histogram and Q-Q plot at the top are not good: in the lower quantiles, the residuals are consistently larger in magnitude than the theoretical expectation based on the assumption of normality. This, of course, is a consequence of the data depending on concentrations in a manifestly nonlinear way. The bottom two plots, though they do show a slight trend in the residuals (due again to the nonlinearity), look surprisingly good. We will come back to the point about nonlinearity in @sec-nonlin-regression.
...
lm(uptake ~ conc * Type * Treatment, data = CO2) |> anova()
lm(uptake ~ conc * Type * Treatment, data = CO2) |> summary()
lm(uptake ~ conc * Type * Treatment, data = CO2) |> diagnosticPlots()
```
