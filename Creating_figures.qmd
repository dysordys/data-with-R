...
@Fauchaldetal2017 tracked the population size of various herds of caribou in North America over time, and correlated population cycling with the amount of vegetation and sea-ice cover. The part of their data that we will use consists of two files: `pop_size.tsv` (data on herd population sizes), and `sea_ice.tsv` (on sea levels of sea ice cover per year and month). You can download them from [this link](https://github.com/dysordys/data-with-R/tree/main/data).
