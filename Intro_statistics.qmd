...
:::{.callout-note}
While this behavior of the dot notation may appear arbitrary, there is method to the madness. In most situations, including the dot within another function indeed means that we would *like* the dot to also be passed as the first argument. For example, we could add row numbers to the bird tibble, by creating a new column whose entries range from one to the number of rows in the data. The number of rows can be obtained with the `nrow` function. Therefore, the following is possible:
