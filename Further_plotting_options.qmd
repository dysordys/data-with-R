...
In the previous examples, the order of text labels was always automatically determined. The basic rule in R is that the ordering follows the alphabet: the default is for `setosa` to precede `versicolor`, which will be followed by `virginica`. This default ordering can be inconvenient, however. Consider the following table of data, with hypothetical temperature measurements (in Celsius degrees) for each month of the year:

```{r}
tibble(month = month.abb, temperature = 10 - 15*cos(seq(0, 2*pi, l=12)))
```
...
The remaining exercises use the Galápagos land snail data ([`island-FL.csv`](https://raw.githubusercontent.com/dysordys/data-with-R/main/data/island-FL.csv)).
