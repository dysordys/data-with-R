...
There is also a very different way of visualizing essentially the same information. The idea behind a *quantile-quantile plot* (Q-Q plot) is that if the residuals are indeed normally distributed, then we can line them up in increasing order along the x-axis, and for each of them, plot the theoretically expected value (based on normality) along the y-axis. If the observed vs. the theoretically predicted values fall on the line matching their values, then there is a perfect correspondence between theory and observation, and the residuals are normally distributed. The stronger the deviation from the line, the more non-normal the residuals are. Below is the residual Q-Q plot for the linear regression on *Naesiotus nux:*
