...
summary(nuxModel) # Obtain regression table
```
...
As advertised, it is possible (and important) to check whether the assumptions behind linear regression actually hold. There is a convenient visual way to do so, via *diagnostic plots*. Here we will be looking at the most commonly used diagnostics.

The simplest one is to look at a histogram of the residuals. This can reveal at a glance whether the residuals are centered at zero, and whether they follow a symmetric, approximately bell-shaped curve (normal distribution):
...
The `residuals` function extracts the residuals from a model object created by `lm`. It returns them as a vector of values---that is why we then feed the result to `as_tibble`, which converts this vector into a tibble with a single column called `value`.

There is also a very different way of visualizing essentially the same information. The idea behind a *quantile-quantile plot* (Q-Q plot) is that if the residuals are indeed normally distributed, then we can line them up in increasing order along the x-axis, and for each of them, plot the theoretically expected value (based on normality) along the y-axis. If the observed vs. the theoretically predicted values fall on the one-to-one line, then there is a perfect match between theory and observation, and the residuals are normally distributed. The stronger the deviation from the one-to-one line indicating a perfect theoretical match, the more non-normal the residuals are. Below is the residual Q-Q plot for the linear regression on *Naesiotus nux:*

```{r}
nuxResidualData <-
  tibble(
    residuals = residuals(nuxModel),
    # The response variable is the first column in the model field of nuxModel:
    y = nuxModel$model[,1],
    # Add also the fitted values as a separate column (for future convenience):
    yHat = fitted(nuxModel)
  )

ggplot(nuxResidualData) +
...
The Q-Q plot can be *much* more revealing than the histogram when we have limited data. As a rough rule, the histogram will only be able to serve as a quick sanity check unless we have at least around 100 or so observations.

Next, it is very useful to display the residuals against the fitted values. If the points have no trends of increase, decrease, valleys, or humps, and the spread of the points is roughly constant, then they adhere to the assumptions of linear regression. In this case, it would be hard to argue that any trend is due to something else than chance:
...
Finally, there is a diagnostic plot that is a slight variation on the above. The difference is that it works with the absolute values of the residuals. This is done because, since residuals will by definition be more or less symmetrically distributed around zero, one can effectively double the precision of the diagnostic by focusing only on magnitudes. For statistical reasons that do not concern us here, the square roots of these absolute values tend to behave much better, which is why we see the square root being taken along the y-axis:

```{r}
ggplot(nuxResidualData) +
  aes(x = yHat, y = sqrt(abs(residuals))) +
  geom_point(color = "steelblue", alpha = 0.7) +
  labs(x = "Estimated Values", y = expression(sqrt("|Residuals|"))) +
  theme_bw()
```

This plot is good for spotting whether the variance of the residuals depends on the fitted values. To satisfy the requirements of linear regression, there should be no such dependence.^[In technical terms, the variation should be *homoscedastic* (be approximately the same for all values of the predictor) as opposed to *heteroscedastic*. This is the same as saying that the variance of the residuals $\epsilon_i$ should be independent of $i$.]

Combining the plots above, we can write a hand-crafted custom function which takes a linear model object and generates all four plots (see the box below for its source code). We will be relying on this function from here on.

:::{.callout-note}
Below is the function we will use to generate diagnostic plots. You can copy and save it in a file (say, `diagnostics.R`) in your working directory, and load it via `source("diagnostics.R")` at the top of any script. That way you will not need to type it in every time. What the `source()` function does is simply run the specified R script, which will then put the function in the computer's memory and ready to be used.

The function uses the `patchwork` package to arrange the four plots in a grid. In case you do not yet have it installed, you can get it via `install.packages("patchwork")`.
...
Using this new `diagnosticPlots` function, we can recreate all four plots in one:
...
How do we interpret these diagnostics? Let us proceed one by one. The top left histogram does not raise any alarms: it peaks at zero and has an approximately symmetric bell shape. Going beyond this, the top right quantile-quantile plot reveals that the residuals follow the theoretically expected normal distribution beautifully, across the full range of its values. The bottom left plot is as good as it can get: there is a blurb of points scattered around zero, with no systematic increase or decrease. Similarly, the bottom right plot is excellent, revealing that the spread of the residuals does not depend on the fitted values. In short, these diagnostics show that the assumptions behind the linear regression model are nicely fulfilled.
...
Looking at the top left, we see that the histogram appears to have a local minimum around zero, with two flanking peaks. It therefore does not have a nice bell-shape. The suspicion that the residuals do not follow a normal distribution is further confirmed by the top right (Q-Q) plot: as seen, small residuals are consistently overestimated (the theoretical values are larger than the actual ones), and large residuals are underestimated. The residual plot in the bottom left suggests that the variance of the residuals is higher for smaller fitted values than for larger ones---and this is reinforced by the scale-location plot in the bottom right. Taken together, this means that any results obtained from a linear regression ought to be treated with caution. Possibly, other methods would be needed to understand these data.
...
Blindly reading off the p-values without considering the diagnostic plots might lead one to take them seriously. This would be wrong however, as the assumptions of the linear regression are clearly not fulfilled. The bottom plots show that the residuals are not independent, and certainly not homoscedastic. The top two ones additionally show that the residuals are not even normally distributed.
...
As before, the bottom two diagnostic plots show that the residuals are not at all independent. Finally, in set 4, the whole regression is based on a single point whose predictor `x` is different from that of the rest:
