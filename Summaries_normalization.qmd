...
The function `pivot_longer` takes three arguments in addition to the first (data) argument that we may pipe in, like above. First, `cols` is the list of columns to be converted into key-value pairs. It uses the same tidy selection mechanisms as the function `select` (@sec-select). This means that one can refer to the columns by number: `cols = 3:5` is the same as `cols = c(3, 4, 5)` and selects the third, fourth, and fifth columns (the ones corresponding to the population densities). One can also write `cols = c(species1, species2, species3)` instead, choosing columns by their names. This can give greater clarity, albeit at the cost of more typing. Or---perhaps the best and cleanest in this case---one can use `cols = starts_with("species")` to select those columns which start with the string `"species"`.

Second, the argument `names_to` in the function `pivot_longer` is the name of the new key column. And third, `values_to` is the name of the new value column.
