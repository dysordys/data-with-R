...
One thing to know about R is that even elementary operations are treated as function calls internally. When we write down something as innocuous as `2 + 5`, what really happens is that R calls the *function* called `+`, with arguments 2 and 5. In fact, we can write it that way too: `2 + 5` is completely equivalent to writing `` `+`(2, 5) ``. Note the back ticks around `` `+` ``: these are required because `+` is not a letter, number, or underscore. Whenever we write down `2 + 5`, the system internally converts it into `` `+`(2, 5) `` first, and then proceeds with the execution. Thus, the fact that we can add two numbers by writing `2 + 5` is just a convenience, to help with writing addition in a way that we tend to be more used to. Such constructions have a name: they are called *syntactic sugar*. Writing `2 + 5` is therefore syntactic sugar for the actual internal form `` `+`(2, 5) ``. The former exists only because `` `+`(2, 5) `` looks strange given our existing arithmetic notational conventions.

As might be guessed from the above, the same holds for all other elementary operations: `` `-` ``, `` `*` ``, `` `/` ``, and `` `^` `` are also functions in R. This means that, e.g., writing `` `-`(`^`(2, 3), `*`(4, 2)) `` is the same as `2^3 - 4*2`. To the computer, they are equivalent---but for humans, the latter form is much more comprehensible.
...
::: {.callout-tip}
One can create the pipe symbol `%>%` with the keyboard shortcut Ctrl+Shift+M (Cmd+Shift+M on a Mac).
:::
