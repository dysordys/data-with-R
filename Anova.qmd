...
y_i
= \beta_0
+ \beta_1 (\text{group is trt1})_i
+ \beta_2 (\text{group is trt2})_i
+ \epsilon_i
$$ {#eq-aov1way3group}
...
It may be clear from the above, but it is good to point out nevertheless, that one does not need to specify the individual indicator variables $(\text{group is trt1})_i$, $(\text{group is trt2})_i$, etc. by hand. Instead, when defining the model formula as `weight ~ group`, R automatically assigns them based on the entries of `group`, and call the corresponding coefficients $\beta_1$ and $\beta_2$ by the names `grouptrt1` and `grouptrt2`. This is very convenient, but it is important to be aware that this substitution is what really goes on under the hood when performing an ANOVA.
