...
While this method of using repeated Wilcoxon tests worked fine above, there are some problems with this approach. One is that it can quickly get out of hand, because having *n* groups means there will be *n* (*n* - 1) / 2 pairs to consider. For instance, if the number of groups is 12 (not a particularly large number in fact), then there are 66 unique pairs already. It would not be pleasant to perform this many tests, even if any single test is quite simple to implement.
