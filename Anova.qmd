...
It may be clear from the above, but it is good to point out nevertheless, that one does not need to specify the indicator variables ($T_i$ and $U_i$ in @eq-aov1way3group) by hand. Instead, when defining the model formula as `weight ~ group`, R will automatically assign them based on the entries of `group`, and call the corresponding coefficients $\beta_T$ and $\beta_U$ by the names `grouptrt1` and `grouptrt2`. This is very convenient, but it is important to be aware of what really goes on under the hood when performing an ANOVA.
