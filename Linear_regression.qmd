...
*Linear regression* was invented by Sir Francis Galton (half-cousin to Charles Darwin), in relation to a very specific problem. He was studying the relationship between the body heights of humans and their children. We can follow his lead and look at a dataset ([`Pearson.tsv`](https://raw.githubusercontent.com/dysordys/data-with-R/main/data/Pearson.tsv)) which is similar to the one used by Sir Francis, but was in fact gathered by Karl Pearson in 1903. The fact that Pearson's data strictly focuses on fathers and their sons, excluding mothers and daughters, contributes an extra air of historical authenticity to the analysis:

```{r}
#| message: false
library(tidyverse)

heightDat <- read_tsv("Pearson.tsv")
print(heightDat)
```

All height measurements are in inches. Displaying the above in a graph yields the following:

```{r}
#| message: false
ggplot(heightDat, aes(x = Father, y = Son)) +
...
```{r}
#| message: false
meanFather <- mean(heightDat$Father) # Average height of fathers
meanSon <- mean(heightDat$Son) # Average height of sons
ic <- meanSon - meanFather # Calculate intercept, assuming slope of 1
print(ic) # Interestingly, the intercept is almost perfectly 1
...
One can check whether these assumptions hold with diagnostic plots. In this particular example, we have a nearly perfect match-up between theory and the data, so the statistical results for the linear regression should be reliable:
...
lm(Son ~ Father, data = heightDat) |>
  autoplot(smooth.colour = NA, colour = "steelblue", alpha = 0.3) +
  theme_bw()
```
